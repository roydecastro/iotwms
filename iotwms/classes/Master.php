...
Class Master extends DBConnection {
	...
	function save_sales(){
		...
		if($save){
			...
			if($sql2){
				$this->settings->set_flashdata("success", " Student Transaction successfully saved");
				$resp['status'] = 'success';
			}else{
				$resp['status'] = 'failed';
	...
	function delete_sales(){
		...
		if($del)
		{
			$this->settings->set_flashdata('success',' Student Transaction successfully deleted.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
